...
# cur_HAIG.execute("DROP TABLE IF EXISTS mapmatching_ONLY_UNIBS_march_2019 CASCADE")
# conn_HAIG.commit()
...
trips_UNIBS_march = pd.read_csv('trips_MARCH_UNIBS.csv')
trips_UNIBS_november = pd.read_csv('trips_NOVEMBER_UNIBS.csv')
...
# idtrajectory = 21509150    # November
...
## read each TRIP going to the University of Brescia (Dipartmento di Ingegneria, Via Brianze, Brescia)
...
def func(arg):
    ...
    if len(viasat) > 5:
        ...
        # grafo = ox.graph_from_polygon(viasat_extent.geometry[0], network_type = 'drive')  # custom_filter=None
        ## ox.plot_graph(grafo)
