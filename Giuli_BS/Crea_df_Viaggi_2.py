...
Fl_Diag  = True    # = True --> stampa diagnostiche  
#           Fl_Tab     stampa la tabella dei viaggi in fase iniziale 
#                                    compresi ACCENSIONI-SPEGNIMENTI
Fl_Tab   = True    # = True --> stampa tabella preliminare e sintetica
#           Fl_Tab1   stampa la tabella dei viaggi CORRETTI in fase finale
Fl_Tab1  = True    # = True --> stampa tabella
Fl_Tempi = True    # = True --> stampa i tempi di esecuzione 
Fl_Tab_Notte_Giorno=True  # = True --> Stampa soste NOTTE - GIORNO
...
Lon_Min_Ris =  9.77;     Lon_Max_Ris = 10.9
Lat_Min_Ris =  45.1;     Lat_Max_Ris = 46.39996
...
'''
Veic_considerati = [ 2725098 ] '''
for kv in  range( len(Veic_considerati[0:2]) ):
#for kv in  range( 1100, len(Veic_considerati) ):
#for kv in  range( len(Veic_considerati) ):
    
    id_veic = Veic_considerati[kv]
    ...
    #dati1v['Km'] = round(float(dati1v.speed * dati1v.Delta_sec / 3600),4)
    ...
    Fl_Viag, Veic_da_Eliminare = Setta_Fl_Viaggio()
    ...
    dati1v['Id_Viaggi'] = Crea_Id_Viaggi()
...
df_Viag.to_csv('df_Viag.csv', sep=',', endcoding='utf-8')

tempo("==== Sta salvando il file 'INFO_Veic'      ", t0)
INFO_Veic.to_csv('INFO_Veic.csv', sep=',', endcoding='utf-8')

df_Eliminati.to_csv('df_Eliminati.csv', sep=',', endcoding='utf-8')
