...
def Tab_Completa():  
    ...
    print('      Pun.Ini-Fin  [   km ](Fl_Viag) Data_Ini          Dat Fin          S.Prec  Viag.   S.Succ[Ore]  Id_Viag[In]   Id_Viag[Fi]' )
    for k in range( len(Pun_Ini) ):
        In = Pun_Ini[k]; Fi = Pun_Fin[k];
...
def Setta_Soste_Generiche_Notte_e_Giorno(_Fl_Stampa):  
    ...
    for k in range(len(Pun_Fin)-1):
        ...
        if (Ora_Decim_Fine_Viag + Sosta_Succes/3600) > (24+2):
            ...
            durata_viaggio_succes = (dati1v.timedate[Pun_Fin[k+1]] - dati1v.timedate[ Pun_Ini[k+1] ]).value/1e9/3600  #  Ore
...
def Setta_Soste_Specifica(Fine_Sosta_Generica, \
    ...
    LoMin = Coord[0] - D_lon;  LoMax = Coord[0] + D_lon      
    LaMin = Coord[1] - D_lat;  LaMax = Coord[1] + D_lat
...
def Calcola_Coord_Case():
    ...
    for k in range(len(pun_Soste_Notte)):
        ...
        LoMin = Lon - D_lon;  LoMax = Lon + D_lon      # Coor. RIQUADRO
...
def Trova_Luogo(Coord):
    ...
    Lon_Min_Univ = Coord[0] - D_lon;  Lon_Max_Univ = Coord[0] + D_lon      
    Lat_Min_Univ = Coord[1] - D_lat;  Lat_Max_Univ = Coord[1] + D_lat
...
def Plot_mappa(Nfig, Titolo, Puntatore, Simbolo, Colore):  
    ...
    ax__.plot(  [Lon_Min_Ris, Lon_Min_Ris, Lon_Max_Ris, Lon_Max_Ris, Lon_Min_Ris], [Lat_Min_Ris, Lat_Max_Ris, Lat_Max_Ris, Lat_Min_Ris, Lat_Min_Ris], 'm' )
    #____________________________  Riquadro INTERNO
    ax__.plot(  [Lon_Min_Int, Lon_Min_Int, Lon_Max_Int, Lon_Max_Int, Lon_Min_Int], [Lat_Min_Int, Lat_Max_Int, Lat_Max_Int, Lat_Min_Int, Lat_Min_Int], '.--g' )
    
    #____________________________  Riquadro UNIVERSITA
    ax__.plot(  [Lon_Min_Univ, Lon_Min_Univ, Lon_Max_Univ, Lon_Max_Univ, Lon_Min_Univ], [Lat_Min_Univ, Lat_Max_Univ, Lat_Max_Univ, Lat_Min_Univ, Lat_Min_Univ], '.--m', linewidth=4 )
...
def Aggiungi_1Veic_ai_Vett_di_INFO_Veic():
    ...
    _Km_Percorsi   .append( float(dati1v.Km_prog[-1:]) )
...
def Aggiungi_1Veic_Eliminato(Causa_eliminaz, _N_elim):
    ...
    # RITORNA:      _N_elim 
    #========================================================================== 
        print('*'*25, _N_elim,'°  Veicolo elimin.: ', dati1v.idterm[0], '  Scartato con codice:', Veic_da_Eliminare, '( ', Causa_eliminaz,')' )    
